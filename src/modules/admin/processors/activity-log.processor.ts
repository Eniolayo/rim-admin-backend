...
import { ActivityLogData } from '../services/activity-queue.service';
import { defaultActivityLogConfig } from '../config/activity-log.config';

@Processor('activity-logs', {
  concurrency: defaultActivityLogConfig.concurrency,
})
  ...
  implements OnModuleDestroy
{
  ...
  private batchBuffer: ActivityLogData[] = [];
  private batchTimeout: NodeJS.Timeout | null = null;
  private readonly BATCH_SIZE = defaultActivityLogConfig.batchSize;
  ...
  async process(job: Job<ActivityLogData>): Promise<void> {
    const { data } = job;

    // Add to batch buffer (thread-safe with concurrency: 1)
  ...
  private async flushBatch(): Promise<void> {
    ...
    try {
      ...
      await this.activityLogRepository.bulkCreate(batch);

      this.logger.debug(
        `Successfully processed batch of ${batchLength} activity logs`,
      );
    } catch (error) {
      this.logger.error(
  ...
  onFailed(job: Job, error: Error) {
    this.logger.error(
      `Activity log job ${job.id} failed: ${error.message}`,
      error.stack,
    );
  }
