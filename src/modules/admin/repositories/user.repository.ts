...
export class AdminMgmtUserRepository {
  ...
  async findWithFilters(filters: { role?: string; status?: string; search?: string }): Promise<AdminUser[]> {
    try {
      this.logger.debug(`Finding admin users with filters: ${JSON.stringify(filters)}`)
      
      const qb = this.repository.createQueryBuilder('admin')
      
      if (filters.role) {
        // Change from exact match to partial match (LIKE)
        qb.andWhere('LOWER(admin.role) LIKE LOWER(:role)', { role: `%${filters.role}%` })
      }
      ...
      if (filters.search) {
        qb.andWhere('(LOWER(admin.username) LIKE :q OR LOWER(admin.email) LIKE :q)', { 
      ...
      this.logger.error(`Error finding admin users with filters: ${error.message}`, error.stack)
  ...
  async countByRole(roleId: string): Promise<number> {
    try {
      ...
      this.logger.error(`Error counting admin users by role: ${error.message}`, error.stack)
      throw new InternalServerErrorException('Failed to count admin users by role')
    }
  }

  async updateStatus(id: string, status: AdminUserStatus): Promise<void> {
    try {
      this.logger.debug(`Updating admin user status: id=${id}, status=${status}`)
      ...
      this.logger.error(`Error updating admin user status: ${error.message}`, error.stack)
      throw new InternalServerErrorException('Failed to update admin user status')
    }
  }
}
