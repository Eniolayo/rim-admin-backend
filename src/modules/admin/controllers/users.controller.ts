...
export class UsersController {
  ...
  @ApiQuery({ name: 'role', required: false, description: 'Filter by role name (case-insensitive)' })
  @ApiQuery({ name: 'status', required: false, description: 'Filter by status (active, inactive, suspended)' })
  @ApiQuery({ name: 'search', required: false, description: 'Search by username or email (case-insensitive)' })
  ...
  async list(@Query() q: AdminUserFiltersDto): Promise<AdminUserResponseDto[]> {
    this.logger.log(`GET /admin/users - Filters: role=${q.role}, status=${q.status}, search=${q.search}`)
  ...
  async updateStatus(@Param('id') id: string, @Body() body: UpdateAdminStatusDto): Promise<AdminUserResponseDto> {
    this.logger.log(`PATCH /admin/users/${id}/status - Updating status to ${body.status}`)
    
    try {
      await this.usersRepo.updateStatus(id, body.status)
      const allUsers = await this.service.list({ role: undefined, status: undefined, search: undefined })
      ...
      this.logger.log(`Successfully updated admin user status: id=${id}, status=${body.status}`)
      return user
    } catch (error) {
