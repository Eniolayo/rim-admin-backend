...
export class SupportController {
  ...
  getTickets(@Query() filters: TicketFiltersDto): Promise<TicketResponseDto[]> {
    this.logger.log(
      `GET /support/tickets - Listing tickets with filters: status ${filters.status}, priority ${filters.priority}, category ${filters.category}, assignedTo ${filters.assignedTo}, department ${filters.department}, search ${filters.search}, dateFrom ${filters.dateFrom}, dateTo ${filters.dateTo}, customerId ${filters.customerId}, escalatedTo ${filters.escalatedTo}`,
    );
  ...
  updateTicket(
    ...
    this.logger.log(
      `PATCH /support/tickets/${id} - Updating ticket with id ${id} by user id ${user.id} and user username ${user.username} with dto ${JSON.stringify(dto)}`,
    );
  ...
  assignTicket(
    ...
    this.logger.log(
      `POST /support/tickets/assign - Assigning ticket with id ${dto.ticketId} to agent id ${dto.agentId} by user id ${user.id} and user username ${user.username}`,
    );
  ...
  escalateTicket(
    ...
    this.logger.log(
      `POST /support/tickets/escalate - Escalating ticket with id ${dto.ticketId} to agent id ${dto.agentId} and admin id ${dto.adminId} with reason ${dto.reason} by user id ${user.id} and user username ${user.username}`,
    );
  ...
  getMessages(@Param('id') id: string): Promise<ChatMessageDto[]> {
    this.logger.log(
      `GET /support/tickets/${id}/messages - Getting messages for ticket id ${id}`,
    );
  ...
  sendMessage(
    @Param('id') id: string,
    @Body() bodyDto: SendMessageBodyDto,
    @CurrentUser() user: AdminUser,
  ): Promise<ChatMessageDto> {
    this.logger.log(
      `POST /support/tickets/${id}/messages - Sending message for ticket id ${id} by user id ${user.id} and user username ${user.username}`,
    );
  ...
  getDepartment(@Param('id') id: string): Promise<DepartmentDto> {
    this.logger.log(
      `GET /support/departments/${id} - Getting department by id ${id}`,
    );
  ...
  createDepartment(@Body() dto: CreateDepartmentDto): Promise<DepartmentDto> {
    this.logger.log(
      `POST /support/departments - Creating department with name ${dto.name} and description ${dto.description} and tier ${dto.tier}`,
    );
  ...
  updateDepartment(
    ...
    this.logger.log(
      `PATCH /support/departments/${id} - Updating department with id ${id} with dto ${JSON.stringify(dto)}`,
    );
  ...
  deleteDepartment(@Param('id') id: string): Promise<{ message: string }> {
    this.logger.log(
      `DELETE /support/departments/${id} - Deleting department with id ${id}`,
    );
    return this.service
      .deleteDepartment(id)
      .then(() => ({ message: 'Department deleted successfully' })) as any;
  }
  ...
  history(@Param('id') id: string): Promise<TicketHistoryDto[]> {
    this.logger.log(
      `GET /support/tickets/${id}/history - Getting ticket history for ticket id ${id}`,
    );
  ...
  notify(@Param('id') id: string, @Body() body: { type: 'email' | 'sms' }) {
    this.logger.log(
      `POST /support/tickets/${id}/notify - Sending ${body.type} notification for ticket id ${id}`,
    );
  ...
  bulkAssign(@Body() dto: BulkAssignDto) {
    this.logger.log(
      `POST /support/tickets/bulk/assign - Bulk assigning ${dto.ticketIds.length} tickets with ids ${dto.ticketIds.join(', ')} to agent id ${dto.agentId}`,
    );
  ...
  bulkResolve(@Body() dto: BulkResolveDto) {
    this.logger.log(
      `POST /support/tickets/bulk/resolve - Bulk resolving ${dto.ticketIds.length} tickets with ids ${dto.ticketIds.join(', ')}`,
    );
  ...
  bulkStatus(@Body() dto: BulkStatusDto) {
    this.logger.log(
      `POST /support/tickets/bulk/status - Bulk updating status to ${dto.status} for ${dto.ticketIds.length} tickets with ids ${dto.ticketIds.join(', ')}`,
    );
  ...
  bulkNotify(@Body() dto: BulkNotifyDto) {
    this.logger.log(
      `POST /support/tickets/bulk/notify - Bulk notifying ${dto.ticketIds.length} tickets with ids ${dto.ticketIds.join(', ')} with type ${dto.type}`,
    );
  ...
  bulkEscalate(@Body() dto: BulkEscalateDto) {
    this.logger.log(
      `POST /support/tickets/bulk/escalate - Bulk escalating ${dto.ticketIds.length} tickets with ids ${dto.ticketIds.join(', ')} to agent id ${dto.agentId} and admin id ${dto.adminId}`,
    );
