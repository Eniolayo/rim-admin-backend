...
export class TicketRepository {
  ...
  async findById(id: string): Promise<SupportTicket | null> {
    ...
    if (ticket) {
      this.logger.log(
        `Ticket found with id ${id} and ticket number ${ticket.ticketNumber}`,
      );
    } else {
  ...
  async findAll(filters: {
    ...
    this.logger.log(
      `Finding all tickets with filters: status ${filters.status}, priority ${filters.priority}, category ${filters.category}, assignedTo ${filters.assignedTo}, department ${filters.department}, search ${filters.search}, dateFrom ${filters.dateFrom}, dateTo ${filters.dateTo}, customerId ${filters.customerId}, escalatedTo ${filters.escalatedTo}`,
    );
    const qb = this.repository.createQueryBuilder('t');
    if (filters.status && filters.status.length)
      qb.andWhere('t.status IN (:...status)', { status: filters.status });
    if (filters.priority && filters.priority.length)
      qb.andWhere('t.priority IN (:...priority)', {
        priority: filters.priority,
      });
    if (filters.category && filters.category.length)
      qb.andWhere('t.category IN (:...category)', {
    ...
    if (filters.department)
      qb.andWhere('t.department = :dept', { dept: filters.department });
    if (filters.customerId)
      qb.andWhere('t.customerId = :customerId', {
        customerId: filters.customerId,
      });
    if (filters.escalatedTo)
      qb.andWhere('t.escalatedTo = :escalatedTo', {
  ...
  async save(entity: SupportTicket): Promise<SupportTicket> {
    this.logger.log(
      `Saving ticket with ticket number ${entity.ticketNumber} and id ${entity.id} and customer id ${entity.customerId} and customer name ${entity.customerName} and customer email ${entity.customerEmail} and customer phone ${entity.customerPhone} and status ${entity.status} and priority ${entity.priority} and category ${entity.category}`,
    );
    const saved = await this.repository.save(entity);
    this.logger.log(
      `Ticket saved successfully with id ${saved.id} and ticket number ${saved.ticketNumber}`,
    );
    return saved;
  }
  ...
  async update(id: string, patch: Partial<SupportTicket>): Promise<void> {
    this.logger.log(
      `Updating ticket with id ${id} with patch ${JSON.stringify(patch)}`,
    );
  ...
  async bulkUpdate(
    ...
    this.logger.log(
      `Bulk updating ${ids.length} tickets with ids ${ids.join(', ')} with patch ${JSON.stringify(patch)}`,
    );
    const items = await this.repository.find({ where: { id: In(ids) } });
    const updated = items.map((i) => ({ ...i, ...patch }));
    await this.repository.save(updated);
    this.logger.log(
      `Bulk update completed successfully for ${updated.length} tickets`,
    );
  }
  ...
  async generateTicketNumber(): Promise<string> {
    ...
    const count = await this.repository
      ...
      .where('t.ticketNumber LIKE :p', { p: `TKT-${year}-%` })
      .getCount();
    const seq = String(count + 1).padStart(3, '0');
    const ticketNumber = `TKT-${year}-${seq}`;
    this.logger.log(
      `Generated ticket number ${ticketNumber} for year ${year} with sequence ${seq} and count ${count}`,
    );
    return ticketNumber;
  }
  ...
  async stats(): Promise<{
    ...
    const stats = {
      ...
      escalatedTickets: tickets.filter(
        (t) => t.status === TicketStatus.ESCALATED,
      ).length,
      unassignedTickets: tickets.filter(
        (t) =>
          t.assignedTo === null ||
      ...
      ticketsByPriority: {
        urgent: tickets.filter((t) => t.priority === TicketPriority.URGENT)
          .length,
      ...
      ticketsByCategory: {
        ...
        billing: tickets.filter((t) => t.category === TicketCategory.BILLING)
          .length,
        account: tickets.filter((t) => t.category === TicketCategory.ACCOUNT)
          .length,
        loan: tickets.filter((t) => t.category === TicketCategory.LOAN).length,
        general: tickets.filter((t) => t.category === TicketCategory.GENERAL)
          .length,
        transaction: tickets.filter(
          (t) => t.category === TicketCategory.TRANSACTION,
        ).length,
      },
    };
    this.logger.log(
      `Ticket statistics calculated: total ${stats.totalTickets}, open ${stats.openTickets}, in progress ${stats.inProgressTickets}, resolved ${stats.resolvedTickets}, escalated ${stats.escalatedTickets}, unassigned ${stats.unassignedTickets}, avg resolution time ${stats.avgResolutionTime} hours, today ${stats.ticketsToday}, this week ${stats.ticketsThisWeek}`,
    );
    return stats;
  }
}
