...
describe('AuthController', () => {
  ...
  it('verify setup returns JWT tokens after enabling 2FA', async () => {
    (service.verify2faSetup as any).mockResolvedValue({
      token: 'jwt-token',
      refreshToken: 'jwt-refresh',
      expiresIn: '1h',
    });
    const res = await controller.verifySetup({
      sessionToken: 'sess',
      code: '123456',
    });
    expect(res.token).toBe('jwt-token');
    expect(res.refreshToken).toBe('jwt-refresh');
  ...
  it('regenerate backup codes returns codes array', async () => {
    ...
    const res = await controller.regenerateBackupCodes({
      id: 'u',
      email: 'e',
      username: 'n',
      role: 'r',
  ...
  it('consume backup code returns token', async () => {
    (service.consumeBackupCode as any).mockResolvedValue({
      token: 't',
      expiresIn: '1h',
    });
    const res = await controller.consumeBackupCode({
      temporaryHash: 'th',
      code: 'code',
    });
