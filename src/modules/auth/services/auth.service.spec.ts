...
describe('AuthService', () => {
  ...
  it('login returns setup-required when 2FA disabled', async () => {
    (adminRepo.findByEmail as any).mockResolvedValue({
      id: 'u',
      email: 'e',
      username: 'n',
      role: 'r',
  ...
  it('start2faSetup returns otpauthUrl and backup codes', async () => {
    (pendingRepo.findActiveByHash as any).mockResolvedValue({
      id: 's1',
      type: 'setup',
  ...
  it('verify2faSetup enables 2FA and returns JWT tokens', async () => {
    (pendingRepo.findActiveByHash as any).mockResolvedValue({
      id: 's1',
      type: 'setup',
      secret: 'secret',
    ...
    (adminRepo.findById as any).mockResolvedValue({
      id: 'u',
      email: 'e',
      username: 'n',
      role: 'r',
  ...
  it('consumeBackupCode issues JWT on valid backup code', async () => {
    ...
    (backupRepo.findActiveByUser as any).mockResolvedValue([
      { id: 'b1', codeHash: hash, used: false },
    ]);
    const spyCompare = jest
      .spyOn(bcrypt, 'compare')
      .mockImplementation(async (plain, hashed) => plain === code);
